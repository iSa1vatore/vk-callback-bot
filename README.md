...
В этой библиотке есть все что вам потребуется для взаимодействия с пользователем в чате сообщества.
...
```
access_token - Ключ доступа к сообществу, получается в настройках>Работа c API>Ключи доступа. 

Создаем и выдаем все нужные права после чего придет смс на телефон, вводим и получаем токен.
...
WebServer - обязательно нужно указать, используется для того, чтобы отправить VK ответ и продоложить выполнение скрипта. Например если вы будете грузить большой фаил пользователю, то VK вывалит TimeOut на запрос и пошлет его повторно.

Далее переходим в раздел "Работа с API" выбираем версию 5.80 и указываем адрес, например https://example.com/index.php, но не нажимаем подтвердить :)

Добавлям обработку событий после объекта:
```php
switch ($bot->event) {
	case 'confirmation':
...
Чтобы отправить простое текствое сообщение нужно выполнить след. команду:
...
$bot->message
  ->keyboard()
    ...
    ->row()
      ->button('Кнопка 1 в во втором ряду', 'positive', [])
      ->button('Кнопка 2 в во втором ряду', 'positive', [])
    ->row()
...
Для создания массива клавитатуры вызовите ->keyboard()
...
Помещение клавитуры в переменную и отправка:
```php
...
//Рекомендую использовать когда пользовалю нужно отправить большой фаил и нужно чтобы VK не выбил TimeOut.
$bot->sendOK(false);
